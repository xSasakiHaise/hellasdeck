...
public class DeckFeaturesCommand {
    public static void register(CommandDispatcher<CommandSource> dispatcher, HellasDeckInfoConfig infoConfig) {
        dispatcher.register((LiteralArgumentBuilder)Commands.func_197057_a("hellas").then(Commands.func_197057_a("deck").then(Commands.func_197057_a("features").executes((ctx) -> {
            if (!infoConfig.isValid()) {
                ...
                infoConfig.getAsJsonArray("features").forEach((f) -> sb.append(f.getAsString()).append("\n"));
